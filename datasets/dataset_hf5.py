...
class DataValSet(data.Dataset):
    def __init__(self, root, mean=(128, 128, 128), isReal= False):
        ...
        self.isReal = isReal
        #合成的测试数据集
        self.input_dir = os.path.join(self.root, 'hazy_test_1800_448_png')
        #采集的数据
        #self.input_dir = os.path.join(self.root, '1-1')
        #self.target_lr_dir = os.path.join(self.root, 'LR')

        self.target_dir = os.path.join(self.root, 'color_test_1800_448_png')
        #self.target_dir = os.path.join(self.root, '1-1')
