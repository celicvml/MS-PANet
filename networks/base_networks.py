...
class SKFusion(nn.Module):
    def __init__(self, num_filter, num_ft, kernel_size=4, stride=2, padding=1, bias=True, activation='prelu', norm=None, mode='iter1'):
        ...
        for i in range(self.num_ft):
            self.up_convs.append(
                nn.ConvTranspose2d(num_filter//(2**i), num_filter//(2**(i+1)), kernel_size, stride, padding, bias=bias)
            )
            self.down_convs.append(
                nn.Conv2d(num_filter//(2**(i+1)), num_filter//(2**i), kernel_size, stride, padding, bias=bias)
            )
