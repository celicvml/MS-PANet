...
```bash
MSBDN-DFF/$python test.py --checkpoint models/MSBDN-DFF/1/MSBDN_epoch_50.pkl
跑实验：python test.py --checkpoint models/MSBDN-RDFF/1/MSBDN_epoch_50.pkl
```python test.py --checkpoint models/MSBDN-RDFF/1/MSBDN_epoch_100.pkl
...
跑实验：python train.py --dataset datasets/RESIDE/ --lr 1e-4 --batchSize 1 --model MSBDN-DFF-v1-1 --name MSBDN-DFF
```

3.(Optional) We also provide a more advanced model (**MSBDN-RDFF**) by adopting the Relaxtion Dense Feature Fusion (**RDFF**) module. 
```bash
MSBDN-DFF/$python train.py --dataset datasets/RESIDE/ --lr 1e-4 --batchSize 1 --model MSBDN-RDFF --name MSBDN-RDFF
```
