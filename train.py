...
parser.add_argument('--dataset', default="datasets/dataset_hf5.py/", type=str, help='Path of the training dataset(.h5)')
parser.add_argument('--model', default='MSBDN-RDFF-MSPA', type=str, help='Import which network')
parser.add_argument('--name', default='MSBDN-RDFF-MSPA-SPR', type=str, help='Filename of the training models')
...
training_settings=[
    {'nEpochs': 100, 'lr': 1e-4, 'step': 25, 'lr_decay': 0.1}
]
...
def train(train_gen, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_gen, 1):
        ...
        if iteration % 100 == 0:
            ...
            print("===> Loading Time: {:.6f}; Runing Time:{:.6f}".format(median_time_data, median_time_gpu))#===> Loading Time: 0.280623; Runing Time:0.194584
            print("===> Epoch[{}]({}/{}): Loss{:.4f};".format(epoch, iteration, len(trainloader), mse.cpu()))#===> Epoch[1](100/3000): Loss0.0262;
            med_time_data = []
            med_time_gpu = []
...
print("===> Loading model {} and criterion".format(opt.model))#Loading model MSBDN-DFF-v1-1 and criterion
...
if opt.resume:
    ...
    model = Net.make_model(opt)
    print(get_n_params(model))#31353061
    mkdir_steptraing()


model = model.to("cuda")
print('RGB Model Size: ', sum(p.numel() for p in model.parameters()))
...
for i in range(opt.start_training_step, 2):
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs+1):
        ...
        print("===>Epoch{} Complete: Avg loss is :{:4f}".format(epoch, psnr))#Epoch1 Complete: Avg loss is :0.009407
