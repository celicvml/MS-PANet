...
class MSAModule(nn.Module):
    def __init__(self, inplanes, scale=4, stride=1, stype='normal'):#scale表示s尺度
        """ 
...
"""class MSAModule(nn.Module):
    def __init__(self, inplanes, scale=3, stride=1, stype='normal', hpc_mode="HPC+SPR(Softmax)"):
        
        hpc_mode支持：
        - "HPC": 只做多尺度卷积，没有SPR和Softmax
        - "SPR": 只做单尺度SPR通道权重，无Softmax，scale固定为1
        - "HPC+SPR(no Softmax)": 多尺度卷积+SPR通道权重，但无Softmax归一
        - "HPC+SPR(Softmax)": 多尺度卷积+SPR通道权重+Softmax归一（默认）
        
        super(MSAModule, self).__init__()
        self.width = inplanes
        self.nums = scale if hpc_mode != "SPR" else 1
        self.stride = stride
        assert stype in ['stage', 'normal'], 'stype must be stage or normal'
        self.stype = stype
        ...
        # SPR通道权重学习，只在有SPR时初始化
        if self.hpc_mode in ["SPR", "HPC+SPR(no Softmax)", "HPC+SPR(Softmax)"]:
            self.attention = SPRModule(self.width)
        else:
            self.attention = None

        # Softmax跨尺度归一，仅“HPC+SPR(Softmax)”模式使用
        if self.hpc_mode == "HPC+SPR(Softmax)":
            self.softmax = nn.Softmax(dim=1)
        else:
            self.softmax = None
    ...
    def forward(self, x):
        ...
        # 仅HPC模式直接拼接求和输出
        if self.hpc_mode == "HPC":
            out = torch.cat(feats, 1).sum(dim=1, keepdim=True)
            return out

        # SPR计算通道权重
        attn_weight = [self.attention(feat) for feat in feats]  # list of (B, C, 1, 1)
        attn_weight = torch.cat(attn_weight, dim=1)             # (B, nums*C, 1, 1)
        attn_vectors = attn_weight.view(batch_size, self.nums, self.width, 1, 1)  # (B, nums, C, 1, 1)
...
class ResidualBlock(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.conv2 = MSAModule(inplanes=channels, scale=2, stride=1, stype='normal')
        #消融实验
        #self.conv2 = MSAModule(inplanes=channels, scale=3, stride=1, stype='normal', hpc_mode="HPC")
        #self.conv2 = MSAModule(inplanes=channels, scale=3, stride=1, stype='normal', hpc_mode="SPR")
        #self.conv2 = MSAModule(inplanes=channels, scale=3, stride=1, hpc_mode="HPC+SPR(no Softmax)")
        #self.conv2 = MSAModule(inplanes=channels, scale=3, stride=1, stype='normal', hpc_mode="HPC+SPR(Softmax)")
        self.relu = nn.PReLU()
